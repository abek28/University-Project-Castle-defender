...
enemy_animations = []
enemy_types = ['Jinn', 'Minotaur_02', 'Golem', 'Ogre', 'Orc','Golem_01' ]
...
for enemy in enemy_types:
    ...
    for animation in animation_types:
        ...
        for i in frames:
            ...
            e_w = img.get_width()
            e_h = img.get_height()
            if enemy in [ 'Golem', 'Ogre','Golem_01']:
                w, h = int(e_w * 0.093), int(e_h * 0.097)
            elif enemy in ['Minotaur_02']:
                w, h = int(e_w * 0.145), int(e_h * 0.145)
            elif enemy in [ 'Orc', 'Jinn']:
...
class Enemy(pygame.sprite.Sprite):
    ...
    def update(self, surface, target, bullets, freeze=False):
        ...
        if self.golems:
            ...
            surface.blit(self.image, (self.rect.x - 115, self.rect.y - 180))
        elif self.minotavrs:
            surface.blit(self.image, (self.rect.x - 45, self.rect.y - 17))
        else:
            surface.blit(self.image, (self.rect.x - 55, self.rect.y - 25))
    ...
    def update_animation(self):
        ...
        if pygame.time.get_ticks() - self.update_time > ANIMATION_COOLDOWN:
            self.update_time = pygame.time.get_ticks()
            self.frame_index += 1
        # if the animation has run out then reset bak to the start
